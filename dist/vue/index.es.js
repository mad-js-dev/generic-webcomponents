class f extends HTMLElement {
  ...
  constructor() {
    ...
    const e = document.createElement("div"), i = document.createElement("slot"), t = document.createElement("style");
    t.textContent = `
      :host {
        display: block;
        width: 100%;
      ...
      .collapsible-item__toggle-container {
        ...
        flex-shrink: 0;
      }`, e.appendChild(i), this.shadowRoot.append(t, e);
  }
  ...
  _setupMutationObserver() {
    this._observer = new MutationObserver(async (e) => {
      let i = !1;
      for (const t of e)
        if (t.type === "attributes" && t.attributeName === "reverse-heading") {
          i = !0;
          break;
      ...
      i && await this._updateReverseHeading();
      ...
      try {
        ...
        const i = async (n) => {
          for (const l of n) {
            ...
            const a = l.querySelectorAll(":scope > collapsible-item");
            a.length > 0 && await i(Array.from(a));
            const d = l.querySelectorAll(":scope > collapsible-list");
            d.length > 0 && await t(Array.from(d));
          }
        }, t = async (n) => {
          for (const l of n) {
            if (l === this) continue;
        ...
        s.length > 0 && await i(Array.from(s));
        const o = this.querySelectorAll(":scope > collapsible-list");
        o.length > 0 && await t(Array.from(o)), this.shadowRoot && this.shadowRoot.offsetHeight;
      } catch (e) {
  ...
  _handleItemToggle(e) {
    ...
    const i = e.target;
    i.parentElement === this && (this._isUpdating = !0, this.hasAttribute("accordion") ? i.expanded && this._closeOtherItems(i) : this.hasAttribute("single-item") && i.expanded && this._closeOtherItems(i), this._isUpdating = !1);
  }
  _closeOtherItems(e) {
    if (this.closest("image-collection"))
      return;
    this.querySelectorAll("collapsible-item").forEach((t) => {
      t !== e && (t.expanded = !1);
    });
  }
  _getOpenItems() {
  ...
  _ensureOneItemExpanded() {
    ...
    if (this._getOpenItems().length === 0) {
      const i = this.querySelector("collapsible-item");
      i && i.setAttribute("expanded", "");
    }
  }
}
...
class g extends HTMLElement {
  ...
  _render() {
    ...
    if (e.style.display = "inline-flex", e.style.alignItems = "center", e.style.gap = "0.5rem", e.style.fontFamily = "inherit", e.style.color = "currentColor", this._reverse && (e.style.flexDirection = "row-reverse"), this._icon) {
      const i = this._icon.startsWith("data:") || this._icon.startsWith("http") || this._icon.startsWith("/") || /\.(png|jpg|jpeg|svg|gif|webp)(\?.*)?$/i.test(this._icon), t = i ? document.createElement("img") : document.createElement("span");
      t.style.display = "inline-flex", t.style.alignItems = "center", t.style.justifyContent = "center", t.style.width = "1em", t.style.height = "1em", i ? (t.src = this._icon, t.alt = "", t.loading = "lazy", t.style.objectFit = "contain") : (t.textContent = this._icon, t.style.fontSize = "1em", t.style.lineHeight = "1"), e.appendChild(t);
    }
    if (this._label) {
      const i = document.createElement("span");
      i.textContent = this._label, e.appendChild(i);
    }
...
class p extends HTMLLIElement {
  ...
  _handleClick(e) {
    ...
    const t = !this.hasAttribute("expanded");
    t ? this.setAttribute("expanded", "") : this.removeAttribute("expanded"), this._isExpanded = t, this._updateContentVisibility(), this.dispatchEvent(new CustomEvent("toggle", {
      detail: { expanded: t },
      bubbles: !0,
      composed: !0
    }));
  ...
  _createHeader() {
    ...
    const t = this.hasAttribute("hide-icon"), s = this.getAttribute("icon");
    if (s && !t) {
      const n = document.createElement("span");
      n.className = "collapsible-item__icon", n.textContent = s, n.style.marginRight = "0.5rem", i.appendChild(n);
    }
    const o = this.getAttribute("label") || "";
    if (o) {
      const n = document.createElement("span");
      n.className = "collapsible-item__label", n.textContent = o, i.appendChild(n);
    }
  ...
  _createContent() {
    ...
    const i = this.querySelectorAll(".collapsible-item__header, .collapsible-item__label, .collapsible-item__icon"), t = new Set(i);
    return Array.from(this.childNodes).filter((o) => o.nodeType === Node.ELEMENT_NODE ? !t.has(o) && !o.closest(".collapsible-item__header") : o.nodeType === Node.TEXT_NODE ? o.textContent.trim() !== "" && !o.textContent.includes("▶") && !o.textContent.includes("▼") : !1).forEach((o) => {
  ...
  attributeChangedCallback(e, i, t) {
    if (e === "removeshift")
      this._removeShift = t !== null, this._removeShift ? this.classList.add("collapsible-item--no-padding") : this.classList.remove("collapsible-item--no-padding");
      ...
      if (t !== null)
        ...
        const o = this.getAttribute("icon"), n = document.createElement("span");
        n.className = "collapsible-item__icon", n.textContent = o, n.style.marginRight = "0.5rem";
        const l = this._header.firstElementChild;
        l && l.insertBefore(n, l.firstChild);
      }
    } else if (e === "expanded") {
      const s = this._isExpanded;
      this._isExpanded = t !== null, this._isExpanded !== s && this._updateContentVisibility();
    } else if (e === "icon" && this._header) {
      let s = this._header.querySelector(".collapsible-item__icon");
      t ? (s || (s = document.createElement("span"), s.className = "collapsible-item__icon", this._header.insertBefore(s, this._header.firstChild)), s.textContent = t) : s && this._header.removeChild(s);
    } else if (e === "label" && this._header) {
      let s = this._header.querySelector(".collapsible-item__label");
      s ? s.textContent = t || "" : t && (s = document.createElement("span"), s.className = "collapsible-item__label", s.textContent = t, this._header.appendChild(s));
    }
  }
}
...
async function C() {
  ...
  try {
    const r = await import("./ProductLayout-DCPDeJA_.js");
    u.ProductLayout = r.default || r;
  } catch (r) {
    console.warn("ProductLayout component not found or failed to load", r);
  }
  return u;
}
function w() {
...
class _ extends HTMLElement {
  ...
  _renderItems(e, i = 0) {
    ...
    return t.style.listStyle = "none", t.style.padding = "0", t.style.margin = "0", e.forEach((s) => {
      const o = s.children && s.children.length > 0, n = this._selectedId === s.id, l = document.createElement("li");
      if (l.setAttribute("data-id", s.id), o) {
        l.setAttribute("is", "collapsible-item"), l.setAttribute("data-id", s.id), l.setAttribute("label", s.name);
        const d = this._hasSelectedDescendant(s) || n;
        d && l.setAttribute("expanded", ""), l.setAttribute("icon", d ? "▼" : "▶"), l.style.marginRight = "8px", l.style.transition = "transform 0.2s ease", n && l.classList.add("menu-item--selected");
        ...
        a.className = "menu-item__leaf", n && a.classList.add("menu-item--selected");
        const d = document.createElement("span");
        d.className = "menu-item__label", d.textContent = s.name, a.appendChild(d), a.addEventListener("click", (c) => {
          c.stopPropagation(), this._selectedId = s.id, this._updateSelectedState(), this.dispatchEvent(new CustomEvent("item-selected", {
            detail: {
              id: s.id,
              item: s,
              name: s.name
  ...
  _onItemClick(e) {
    ...
    if (i) {
      ...
      o && (this.selected = o, this._updateSelectedState(), this.dispatchEvent(new CustomEvent("item-selected", {
        detail: { id: o },
        bubbles: !0,
        composed: !0
      })));
    ...
    if (s && s.getAttribute("is") === "collapsible-item") {
      e.stopPropagation();
      const o = s.hasAttribute("expanded"), n = s.querySelector(".menu-item__arrow");
      o ? (s.removeAttribute("expanded"), n && (n.textContent = "▶")) : (s.setAttribute("expanded", ""), n && (n.textContent = "▼")), s.dispatchEvent(new CustomEvent("toggle", {
        detail: { expanded: !o },
        bubbles: !0,
        composed: !0
      }));
...
class E extends HTMLElement {
  ...
  _handleToggle(e) {
    ...
    try {
      ...
      if (i = o.indexOf(s), i === -1 || i === this._currentOpenIndex)
        return;
      t = s.expanded, o.forEach((n, l) => {
        l !== i && n.expanded && (n.expanded = !1);
      }), i === this._currentOpenIndex ? (s.expanded = !1, this._currentOpenIndex = -1) : (s.expanded = !0, this._currentOpenIndex = i);
    } finally {
  ...
  _onItemToggle(e) {
    e.stopPropagation();
    const i = e.target.closest('li[is="collapsible-item"]');
    if (!i) return;
    const t = Array.from(this.shadowRoot.querySelectorAll('li[is="collapsible-item"]')), s = t.indexOf(i);
    if (s !== -1) {
      if (!e.detail.expanded) {
        this._currentOpenIndex === s && (this._currentOpenIndex = -1);
        return;
      }
      if (this._currentOpenIndex !== -1 && this._currentOpenIndex !== s) {
        const o = t[this._currentOpenIndex];
        o && o.toggle(!1);
      }
      this._currentOpenIndex = s;
    }
  }
  render() {
    ...
    y().catch(console.error), Object.entries(A).forEach(([e, i]) => {
      if (typeof i == "function" && i.name) {
        const t = i.is || e.toLowerCase(), s = {
          ...
          props: {},
          render() {
            const o = Object.entries(this.$attrs).reduce((l, [a, d]) => {
              const c = a.replace(/-([a-z])/g, (h) => h[1].toUpperCase());
              return l[c] = d, l;
            }, {}), n = {};
